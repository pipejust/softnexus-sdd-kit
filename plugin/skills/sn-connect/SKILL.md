...
- Copia el motor al repositorio (el CI no tiene el plugin): `scripts/sn/` ← `${CLAUDE_PLUGIN_ROOT}/scripts/{sn-sync.mjs,validation-state.mjs,sn-clave-altum.sh,sn-clave-altum.ps1,sync/}`. Si ya existe, reemplázalo por la versión del plugin (es código generado; no se edita a mano).
