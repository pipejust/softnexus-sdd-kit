...
3e. **Solo si la persona es el líder** (`node scripts/sn/sn-sync.mjs lead` dice "eres tú") y el proyecto no tiene `.github/workflows/sn-sync.yml` (o `azure-pipelines.yml` en Azure): dile en una línea "Como líder, falta configurar el proyecto (5 min) para que las tareas se cierren solas y solo tu aprobación permita unir los PR. ¿Lo hacemos?" y con el sí usa `sn-connect`. **A quien no es el líder nunca se le menciona esto.**
