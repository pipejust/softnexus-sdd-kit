...
**Un proyecto puede tener varios repositorios** (app, web, consola...). Si es el caso, el comando no elige solo: los lista y hay que volver a llamarlo con `--repo <nombre>`. Pregúntale a la persona **cuál** quiere y, si no sabe, muéstrale la lista (`… repos <proyecto>`). Con varios, la carpeta se llama como el repositorio elegido.
