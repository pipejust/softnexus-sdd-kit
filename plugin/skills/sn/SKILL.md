...
0c. **Proyecto que la persona YA tenía clonado** (lo normal: ya había gente trabajando antes de la metodología): **nunca** le propongas clonarlo otra vez ni crear otra carpeta. Primero `git pull` de la rama principal: si alguien ya lo preparó, con eso basta. Si sigue sin `AGENTS.md`/`openspec/`, pasa al punto 1 (se prepara aquí mismo, en su propia rama).
0d. **Trabajo a medias de antes de la metodología** (la persona está en una rama con commits o cambios que no pertenecen a ningún ítem de `docs/items/`): **adóptalo, no lo rehagas.** Pregúntale en una línea qué estaba haciendo; crea el ítem con `branch:` = su rama; escribe el plano con dos partes: *lo que ya está hecho* (tareas marcadas `[x]`, con los commits que lo respaldan) y *lo que falta* (tareas `[ ]`). El sello 1 aprueba ese plano completo (lo hecho también queda revisado). Después sigue el camino normal desde el paso 7. Si tiene cambios sin commit, guárdalos primero con `sn-ship` (modo commit) en su misma rama.
...
3a. **Si la persona no tiene el proyecto en su computador** ("no lo tengo", "clóname el proyecto X"): `node scripts/sn/sn-sync.mjs clone <nombre>`. **Si la carpeta no tiene el motor** (está vacía o el repo no está preparado), usa el del plugin: `node "${CLAUDE_PLUGIN_ROOT}/scripts/sn-sync.mjs" clone <nombre> --in <carpeta>` (y `… projects` para listar los suyos). Lo busca en Altum por nombre y lo clona; luego se trabaja ahí. **Antes de clonar, pregunta SIEMPRE dónde ponerlo** (el comando se niega sin ruta; no inventes una): "¿En qué carpeta lo dejo? Puedes darme una carpeta madre (ej. `~/Proyectos`) y dentro creo la del proyecto, o la ruta exacta donde quieres que quede el contenido (ej. `~/Proyectos/cursos`)." - Carpeta madre → `… clone <nombre> --in ~/Proyectos` (queda en `~/Proyectos/<nombre-del-proyecto>`). - Ruta exacta → `… clone <nombre> --into ~/Proyectos/cursos` (el contenido del repositorio queda ahí, sin otra carpeta dentro). - Aquí mismo → `… clone <nombre> --here` (solo si la carpeta actual está vacía). Si la carpeta madre ya se llama como el proyecto, no se crea otra igual adentro. Si la carpeta elegida ya tiene archivos, no se clona encima: te lo dice y se elige otra.
...
3c. **Si el repo no tiene conector `altum` y la persona sí tiene su clave:** corre `node "${CLAUDE_PLUGIN_ROOT}/scripts/sn-sync.mjs" conectar` sin preguntar nada. La clave dice quién es y el remoto `origin` dice qué repositorio es: si un solo proyecto de Altum lo usa, queda conectado y sigues. Si lo usan varios (ej. un mismo repositorio para First Class, Mi Boleta y Yo Soy Fan), el comando los nombra: pregúntale **por el nombre** "¿En cuál vas a trabajar: First Class, Mi Boleta…?" y corre `… conectar "<nombre>"`. Si ninguno lo usa, pregúntale en cuál de sus proyectos (por nombre) y `conectar "<nombre>"` lo conecta y registra el repositorio. **Nunca le pidas a nadie una "clave del proyecto" ni un nombre corto**: el proyecto sale solo del repositorio. Si el comando falla o no hay clave, sigue sin Altum.
3d. **Si el proyecto de Altum no tiene registrado su repositorio** (el sistema te lo avisa, o `node scripts/sn/sn-sync.mjs repo-check` lo dice): **dilo primero, antes de lo que traiga la persona**, en una línea: "Este proyecto no tiene guardado en Altum de dónde se clona; si lo registro, tus compañeros lo traen por el nombre. ¿Lo hago?". Con el sí: `node scripts/sn/sn-sync.mjs set-repo "<nombre del proyecto>"` (usa el remoto `origin`). **Agrega, no reemplaza**: si el proyecto ya tenía otros repositorios registrados, quedan todos (`… repos <proyecto>` los muestra), y el proveedor (GitHub, Azure DevOps…) se deduce de la dirección. Si alguno quedó mal registrado, se saca con `… quitar-repo <proyecto> <cuál> --si`, **preguntándole antes a la persona**: lo ve todo el equipo (el código no se toca, solo el registro en Altum). Si responde `403`, su clave es anterior al permiso `projects:write`: que la regenere en Altum. Si dice que no, sigue con lo suyo y no insistas en esta sesión.
3e. **Configuración del proyecto (secretos, revisiones automáticas, protección de la rama): solo para el ADMINISTRADOR del repositorio.** Corre `node scripts/sn/sn-sync.mjs pedir-config`: si dice que la persona administra el repositorio y falta configurar, ofrécelo en una línea y, con el sí, usa `sn-connect`. En cualquier otro caso **no digas nada**: ni al equipo ni al líder (el líder del proyecto en Altum casi nunca administra el repositorio ni tiene la clave de la empresa).
...
- Si dice que **el repositorio no está unido a Altum** y la persona tiene clave: conéctalo ahora con `node "${CLAUDE_PLUGIN_ROOT}/scripts/sn-sync.mjs" conectar` (el proyecto sale del repositorio; si lo usan varios, pregunta por nombre) y vuelve a correr `asegurar`. **No construyas sin tarea.**
