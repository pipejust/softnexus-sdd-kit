...
| Tareas de reuniones (Acten) | `GET /tasks` mezcla tareas nativas con las que nacen en reuniones de Acten (`"source": "acten"`, id `acten:…`, casi todos los campos en `null`). El motor las marca como **solo lectura**: salen en `backlog` con la nota "de Acten", `pull` no las trae (las cuenta aparte), `link` las rechaza y un `sync` sobre una tarea así falla con un mensaje claro en vez de escribir. Con `updated_since` no vienen (no tienen fecha), así que el vigilante nunca avisa de ellas. |
