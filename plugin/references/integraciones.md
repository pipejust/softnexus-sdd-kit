...
| Muchas tareas y muchos proyectos | `GET /tasks` y `GET /projects` se piden por páginas (`page`, `limit=200`) hasta traer el `total`; las dos vienen envueltas en `{items, total}`. `/config/estados` y `/config/campos` devuelven **listas sueltas**: el motor acepta las dos formas. |
...
| Registrar el repositorio | `sn-sync set-repo <nombre> [url]` escribe `repo_url` en Altum (`PUT /projects/{id}/repo`); sin url usa el remoto `origin` del repositorio actual. Necesita una clave con **`projects:write`**: las anteriores a ese permiso dan `403` y hay que regenerarlas. |
| Quién firma | Solo el líder que dice Altum, salvo un caso: **si el PR lo abrió el propio líder, su PR ya cuenta como firmado** (es quien decide, y GitHub no deja aprobar el PR propio). Lo comprueba `verificar-firma`, que es lo que corre el CI. |
| Motor al día | El repositorio lleva una **copia** del motor en `scripts/sn` para que el CI pueda correrlo sin el plugin. Esa copia viaja en las ramas del proyecto, así que puede ser más vieja que el plugin de cada persona. **Los hooks del plugin ejecutan siempre el motor del plugin** (la copia solo sirve para saber si el proyecto ya está preparado y para el CI), así que nadie trabaja con un motor viejo. `sn-sync motor` compara las dos y `sn-sync motor --actualizar` deja la del repositorio igual (se entrega con `/sn-ship`). |
| Empezar en un proyecto | `sn-sync projects` lista por **nombre** lo que ve la clave y marca los que no tienen repositorio registrado (a nadie se le pide una "clave del proyecto"). `sn-sync conectar [nombre]` une el repositorio con su proyecto: compara el remoto `origin` (proveedor + `org/repo`, sin importar https/ssh ni mayúsculas) con los repositorios de cada proyecto asignado; si uno solo lo usa, conecta sin preguntar; si varios (un repositorio para varios proyectos), los nombra para preguntar por nombre; si ninguno, con el nombre conecta y registra el repositorio en ese proyecto. `sn-sync clone <nombre>` busca el proyecto (sin tildes ni mayúsculas; nombre exacto gana sobre parecidos, y si hay varios los muestra para elegir) y clona su `repo_url` **donde diga la persona** (sin ruta, se niega y pide preguntar): `--in <carpeta madre>` deja `<carpeta>/<nombre-del-proyecto>`; `--into <ruta>` (o `--here`) pone el contenido en esa ruta exacta, sin crear otra carpeta; si la carpeta madre ya se llama como el proyecto, no se anida otra igual. No clona encima de una carpeta que ya tenga contenido. Ambos funcionan **antes** de que el repositorio esté configurado. |
...
| Cuándo nace la tarea | Al guardar el ítem (`sn.item.created`, con el "sí" a la tarjeta) la sincronización en segundo plano crea la tarea con `external_ref = <ID>`. Como ese envío es silencioso, **antes de construir** la skill `sn` corre `asegurar <ID>`: en primer plano, confirma con `GET /tasks/{id}` que la tarea existe de verdad, la crea si falta y la **vuelve a crear si se borró en Altum** (con otra `Idempotency-Key`, porque con la misma Altum devolvería la tarea borrada). Sin conector o sin clave, lo dice en vez de callarse. Estados después: `new` (triaged/ready) → `active` (desde planning hasta in_review) → **`closed` en cuanto se une el PR** (merged; lo empuja el CI `sn-sync.yml` al cerrarse el PR, o el siguiente `git pull` con el hook post-merge) → `closed` (done, al archivar, sin cambio). **Si el proyecto tiene workflow propio** y le falta alguna de esas llaves, se usa el equivalente por `kind` (al terminar, el ÚLTIMO estado `done`), así una tarea terminada nunca queda abierta. **Al cerrar**, la skill `sn` vuelve a correr `asegurar <ID>`: empuja la etapa actual y confirma "Cerrada en Altum"; si Altum responde `409` por bloqueadores, dice cuál. Sin enlace en el ítem, busca la tarea por `external_ref`. **Historial limpio**: la descripción en Altum solo lleva lo que casi no cambia (historia, riesgo, tamaño, responsable, rama, plano, PR); la etapa la dice el estado y el avance/commits viven en el repo, porque Altum guarda el antes y el después completos de cada cambio. Además cada PATCH lleva **solo los campos que cambiaron** y, si nada cambió, no se envía; crear una tarea es un solo `POST`. Commits del ítem: solo los que llevan `Refs: <ID>` o tocan su plano (no los que apenas mencionan el id). **Descartar**: `descartado: <motivo>` en la cabecera → etapa `discarded` → estado `removed` (kind `cancelled`; si el workflow no tiene, el último `done`) y el motivo al inicio de la descripción. Cada petición lleva `User-Agent: softnexus-sdd/<versión>` y `X-Client-Name: Plugin Softnexus` para que Altum muestre quién escribió (pedido J). **PR en Azure DevOps**: si el remoto es `dev.azure.com`/`visualstudio.com`, el estado del PR sale de su API REST (`active`→abierto, `completed`→unido, `abandoned`→cerrado) con `SN_AZURE_PAT` (Llavero en Mac; `$(System.AccessToken)` en Azure Pipelines), enviado por la entrada estándar de curl. **Criterios de aceptación**: la sección "Criterios de aceptación" del ítem viaja en su propio campo `acceptance_criteria` (texto plano), ya en el `POST`; se actualiza cuando cambia en el repo (si el ítem no tiene criterios, no se borra lo que haya en Altum). Al comparar se quitan HTML y viñetas: si en Altum solo cambió el formato, no se reescribe. Al importar una tarea, sus criterios llegan al ítem limpios de HTML. A las tareas de Acten no se envían (su PATCH no los acepta). |
| Repositorios del proyecto | Un proyecto puede tener **varios** repositorios, y de **distintos proveedores**. Altum los expone en `repos` (dentro de `GET /projects` y en `GET/POST/DELETE /projects/{id}/repos`), además del `repo_url` de siempre; el motor los unifica en una lista. `repos <proyecto>` los muestra con su proveedor, `projects` dice cuántos hay, `set-repo` **agrega** (deduce el proveedor de la dirección; `409` = ya estaba) y `quitar-repo <proyecto> <cuál> --si` lo saca (sin `--si` solo dice qué haría; `404` = no estaba). `clone` no elige solo con varios: los lista y exige `--repo <nombre>`, y la carpeta se llama como el repositorio elegido. **Azure DevOps**: `provider: azure_devops` y el nombre lleva TRES partes (`organizacion/proyecto/repositorio`); con dos, Altum responde `422`. El motor lo deduce de `dev.azure.com/<org>/<proyecto>/_git/<repo>` y de `<org>.visualstudio.com/...`. |
| Tareas de reuniones (Acten) | `GET /tasks` mezcla tareas nativas con las que nacen en reuniones de Acten (`"source": "acten"`, id `acten:…`, `kind: acten-tarea`, varios campos en `null`). Se leen y se traen como cualquier otra: `backlog` las marca "de una reunión", `pull` las importa con id `ACT-<n>` y `updated_since` ya las incluye (traen `updated_at`, sin zona: se lee como UTC), así que el vigilante también avisa de ellas. Al escribir, `PATCH /tasks/acten:<id>` **solo acepta `state`, `assignee_id`/`assignee_email`, `title` y `description`**; cualquier otro campo responde `422`, así que el motor no envía prioridad, etiquetas, `custom_fields` ni `external_ref`. Sus estados son fijos y de Acten (`pending`, `blocked`, `done`, `cancelled`), no los de `/config/estados`: la etapa del ítem se traduce con `acten_status_map` (por defecto, todo es `pending` hasta que se une, y ahí `done`). Un `409` de Acten (p. ej. `cancelled` no admite volver atrás) se reenvía tal cual. No tienen dependencias ni entran en `deleted`. |
...
- **Personal (`sk_user_…`, lo normal):** la genera la propia persona en Altum → Mi perfil → Mis datos → "Tu clave personal de API" → Regenerar. Nace con `tasks:read`, `tasks:write`, `projects:read` y `projects:write` (este último solo en claves generadas desde el 20-sep-2026: sin él, registrar el repositorio da `403`), y alcanza **solo a los proyectos donde está asignada** (en otro: `403 "No estás asignado a este proyecto"`; asignarla o quitarla cambia su acceso al instante). Sus cambios quedan como `updated_by: {type:"user", …}`, así nadie recibe avisos de lo propio.
