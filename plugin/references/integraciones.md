...
| Repositorios del proyecto | Un proyecto puede tener **varios** repositorios (app, web, consola...). Altum lo dice de dos formas que conviven: `repo_url` (uno solo, el de siempre) y `repos` (la lista, con `GET`/`POST /projects/{id}/repos`). El motor las unifica: `repos <proyecto>` los lista, `projects` avisa cuántos hay, `set-repo` agrega a la lista (`409` = ya estaba) y cae a `repo_url` si esa versión de Altum todavía no tiene el endpoint. `clone` **no elige solo** cuando hay varios: los muestra y exige `--repo <nombre>`; la carpeta se llama entonces como el repositorio elegido, no como el proyecto. No hay `DELETE` por API: quitar uno se hace desde Altum. |
